...
class _TimelineStripState extends State<TimelineStrip> {
  ...
  void _updateTimelineProgress() {
    ...
    _scrollDelayTimer = Timer(Duration.zero, () {
      if (mounted) {
        final newProgress =
            _timelineController.eventCount > 0
                ? activeItems / _timelineController.eventCount
                : 0.0;
        setState(() {
                          ...
                          ? ScrollReveal(
                            child: _TimelineTile(
                              ...
                              isActive:
                                  _timelineProgress >=
                                  ((index + 1) /
                                      _timelineController.eventCount),
                              isLeft: index % 2 == 0,
                            ),
                          )
                          : _TimelineTile(
                            ...
                            isActive:
                                _timelineProgress >=
                                ((index + 1) / _timelineController.eventCount),
                            isLeft: index % 2 == 0,
                          ),
                ),
              ],
  ...
  Widget _buildConnectingLine(int previousIndex) {
    ...
    final currentItemProgress =
        (currentItemIndex + 1) / _timelineController.eventCount;

    var lineProgress = 0.0;

    if (_timelineProgress >= currentItemProgress) {
      lineProgress =
          1.0; // Fully active - the dot this line connects to is active
    }
...
class _TimelineTile extends StatelessWidget {
      ...
      child: Row(
        ...
        children: [
          Expanded(
            child:
                isLeft
                    ? _buildTileContent(context, Alignment.centerRight)
                    : const SizedBox(),
          ),
          ...
          Expanded(
            child:
                !isLeft
                    ? _buildTileContent(context, Alignment.centerLeft)
                    : const SizedBox(),
          ),
        ],
      ),
    ...
    return Align(
      ...
      child: AnimatedContainer(
        ...
        margin: EdgeInsets.only(
          left: isLeft ? 0 : horizontalMargin,
          right: isLeft ? horizontalMargin : 0,
        ),
        constraints: BoxConstraints(
          maxWidth: isMobile ? screenWidth * 0.75 : screenWidth * .38,
        ),
        decoration: BoxDecoration(
        ...
        child: Column(
          ...
          children: [
            ...
            if (event.proofs.isNotEmpty) ...[
              SizedBox(height: isMobile ? 8 : 10),
              _buildProofButtons(context, isMobile),
            ],
          ],
        ),
      ),
  ...
  Widget _buildProofButtons(BuildContext context, bool isMobile) => Column(
    crossAxisAlignment:
        isLeft ? CrossAxisAlignment.end : CrossAxisAlignment.start,
    children: [
      Text(
      ...
      Wrap(
        alignment: isLeft ? WrapAlignment.end : WrapAlignment.start,
        spacing: 6,
        runSpacing: 6,
        children: [
          for (final proof in event.proofs)
            Tooltip(
              message: 'Open ${proof.label}',
              child: OutlinedButton.icon(
                onPressed: () => _openProof(proof),
                icon: Icon(Icons.verified_outlined, size: isMobile ? 13 : 14),
  ...
  Future<void> _openProof(CertificateProof proof) async {
    final uri =
        proof.url == null
            ? Uri.base.resolve(proof.webPath!)
            : Uri.parse(proof.url!);
